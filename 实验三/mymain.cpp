...
{
    char ch;
    cout << "LR（1）分析程序，编制人：姓名     学号    班级，班" << endl;
    cout << "请输入分析的字符串：";
    cin >> ch;
    while (ch != '#')
    {
...
void LRAnalyse()
{
    ...
    while (analyseTable.action[s][analyseTable.getTerminalIndex(ch)] != "acc")
    {
        ...
        if (str.substr(0, 1) == "s")
        {
            cout << setw(10) << step << setw(10) << vectTrancStr(0) << setw(10) << vectTrancStr(1) << setw(10) << vectTrancStr(2) << setw(10) << "A"
                 << "CTION[" << status.back() << "," << ch << "]=S" << s << ","
                 << "状态" << s << "入栈" << endl;
            // 输入符号入栈
            sign.push_back(ch);
